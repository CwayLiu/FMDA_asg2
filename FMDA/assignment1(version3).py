...
#m_list 月份数组
#year_range 年份长度
def calMonthAvg(list, m_list, year_range):
    averageList = []
    for month in range(1, 12+1):
        ...
        average = count/year_range
        averageList.append(average)
    return averageList

averageList = calMonthAvg(anomalyList, monthList, 60)
...
pl.show()
#pvalue=0?
print(kstest(frontList, 'norm'))
print(kstest(backList, 'norm'))
#task3(?)
#计算前后三十年的月平均值
front_averageList = calMonthAvg(frontList, front_monthList, 30)
back_averageList = calMonthAvg(backList, back_monthList, 30)
...
#标准差
month_difflist = []
front_stdlist = calMonthStd(frontList, front_monthList)
back_stdlist = calMonthStd(backList, back_monthList)
for i in range(12):
...
print(kstest(month_diffList, 'norm'))
